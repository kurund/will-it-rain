...
GWR["season"] = pd.Categorical(GWR["season"], categories=["Winter", "Spring", "Summer", "Autumn"], ordered=True)
...
def model(GWR, country, date_str, sigma_days=15):
    ...
    def wind_class_kph(k):
        if not np.isfinite(k): return np.nan
        if k < 12: return "Calm/Light"
        if k < 20: return "Light Breeze"
        if k < 28: return "Breezy"
        if k < 39: return "Windy"
        if k < 55: return "Strong Breeze"
        return "Gale"

    def wet_class_mm(mm):
        if not np.isfinite(mm) or mm <= 0: return "Dry"
        if mm < 0.2: return "Drizzle"
        if mm < 1.0: return "Light Rain"
        if mm < 5.0: return "Moderate Rain"
        if mm < 20.0: return "Heavy Rain"
        return "Very Heavy Rain"

    def aq_class_defra(aqi):
        if not np.isfinite(aqi): return np.nan
        if 1 <= aqi <= 3: return "Low"
        if 4 <= aqi <= 6: return "Moderate"
        if 7 <= aqi <= 9: return "High"
        if aqi >= 10: return "Very High"
        return np.nan

    def cloud_desc(pct):
        if not np.isfinite(pct): return None
    ...
    pred_feels_c = wtd_mean(df["feels_like_celsius"].values, w) if "feels_like_celsius" in df.columns else np.nan
    pred_cloud_pct = wtd_mean(df["cloud"].values, w) if "cloud" in df.columns else np.nan

    # --- Predicted condition_text via weighted mode ---
    pred_condition = weighted_mode(df["condition_text"], w) if "condition_text" in df.columns else np.nan
    ...
    parts = []
    if temp_cls: parts.append(temp_cls)
    if wind_cls and wind_cls not in ["Calm/Light", None]: parts.append(wind_cls.lower())
    if wet_cls and wet_cls != "Dry": parts.append(wet_cls.lower())
    ...
    summary_wide = pd.DataFrame([{
        ...
        "Pred_Wind_kph": None if not np.isfinite(pred_wind_kph) else round(pred_wind_kph, 1),
        "Pred_AQ_Index": round(pred_aq_index, 1),
        "Pred_FeelsLike_C": None if not np.isfinite(t_for_feel) else round(t_for_feel, 1),
        "Pred_Cloud_pct": None if not np.isfinite(pred_cloud_pct) else round(pred_cloud_pct, 0),
